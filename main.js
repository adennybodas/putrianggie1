...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
    ...
    const XeonBotInc = makeWASocket({
      ...
      browser: [ 'Mac OS', 'Safari', '' ], // for this issues https://github.com/WhiskeySockets/Baileys/issues/328
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +6282187527485")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +6282187527485 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} YT CHANNEL: ADENNY BODAS`,'cyan'))
            console.log(color(`${themeemoji} GITHUB: ADENNY BODAS `,'red'))
            console.log(color(`${themeemoji} INSTAGRAM: @ADENNYBODAS `,'blue'))
            console.log(color(`${themeemoji} WA NUMBER: ${owner}`,'green'))
